...
void TestValueOne()
{
	...
	Thread *threadInc1 = new Thread((char*)"Inc1");
	Thread *threadInc2 = new Thread((char*)"Inc2");
	Thread *threadDec1 = new Thread((char*)"Dec1");
	Thread *threadDec2 = new Thread((char*)"Dec2");
	threadInc1->Fork(Inc_v1,0,1);
	threadInc2->Fork(Inc_v1,1,1);
	threadDec1->Fork(Dec_v1,2,1);
	threadDec2->Fork(Dec_v1,3,1);
	threadInc1->Join(threadInc2);
	threadInc1->Join(threadDec1);
	threadInc1->Join(threadDec2);
	currentThread->Join(threadInc1);
...
void TestValueMinusOne()
{
	...
	printf("enter TestValueMinusOne, value=%d...\n", value);
        ...
        Thread *threadDec1 = new Thread((char*)"Dec1");
        Thread *threadDec2 = new Thread((char*)"Dec2");
        threadInc1->Fork(Inc_v2,0,1);
        threadInc2->Fork(Inc_v2,1,1);
        threadDec1->Fork(Dec_v2,2,1);
        threadDec2->Fork(Dec_v2,3,1);
        threadInc1->Join(threadInc2);
        threadInc1->Join(threadDec1);
        threadInc1->Join(threadDec2);
        currentThread->Join(threadInc1);
...
void TestConsistency()
{
	...
	Thread *thread = new Thread((char*) "Inc_Consistent1");
	Thread *thread1 = new Thread((char*) "Inc_Consistent2");
	Thread *thread2 = new Thread((char*) "Dec_Consistent1");
	Thread *thread3 = new Thread((char*) "Dec_Consistent2");
	thread->Fork(Inc_Consistent,0,1);
	thread1->Fork(Inc_Consistent,1,1);
	thread2->Fork(Dec_Consistent,2,1);
